...
function addItem($data){
    ...
    $query = "INSERT INTO items (item_name, stock_type_id, price, cost, quantity) VALUES ('$item_name', '$stock_type_id', '$price', '$cost', '$quantity')";
    mysqli_query($conn, $query);
    $affected_rows = mysqli_affected_rows($conn);
    if ($affected_rows > 0) {
...
function sellItem($data){
    ...
    if (!mysqli_query($conn, $query)) {
        echo "Error inserting into sales table: " . mysqli_error($conn);
        return 0;
    }
    $sales_affected_rows = mysqli_affected_rows($conn);
    
    // Update items table
    $query = "UPDATE items SET quantity = quantity - $quantity_sold WHERE id = '$item_id'";
    if (!mysqli_query($conn, $query)) {
        echo "Error updating items table: " . mysqli_error($conn);
        return 0;
    }
    $items_affected_rows = mysqli_affected_rows($conn);

    if ($sales_affected_rows > 0 && $items_affected_rows > 0) {
        logAction($_SESSION["user_id"], "Sold item ID: $item_id, Quantity: $quantity_sold");
    }

    return $sales_affected_rows > 0 && $items_affected_rows > 0 ? 1 : 0;
}

function getSalesReport(){
    global $conn;
    $query = "SELECT sales.*, items.item_name, items.price, items.cost FROM sales LEFT JOIN items ON sales.item_id = items.id";
    return query($query);
}
...
function getNetProfit(){
    global $conn;
    $query = "SELECT SUM(sales.quantity_sold * (items.price - items.cost)) as net_profit FROM sales LEFT JOIN items ON sales.item_id = items.id";
    $result = mysqli_query($conn, $query);
    $data = mysqli_fetch_assoc($result);
...
function register($data){
    ...
    if ($user_id) {
        logAction($user_id, "Registered new user: $username");
    }
